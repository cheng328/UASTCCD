...
for k in ${K_VALUES}; do
  mapper_dir="${OUT_BASE}/k${k}"
  suast="${mapper_dir}/${DATASET}_${SPLIT}_suast.jsonl"
  evidence="${mapper_dir}/${DATASET}_${SPLIT}_evidence.jsonl"
  quality_log="${mapper_dir}/${DATASET}_${SPLIT}_quality_guard.jsonl"
  raw_pred="${mapper_dir}/${DATASET}_${SPLIT}_uast_ccd_raw.jsonl"
  norm_pred="${mapper_dir}/${DATASET}_${SPLIT}_uast_ccd_norm.jsonl"
  keeper_log="${mapper_dir}/${DATASET}_${SPLIT}_standard_keeper.jsonl"
  ...
  python scripts/12_eval_metrics.py \
    --input "${norm_pred}" \
    --output "${mapper_dir}/metrics.csv"
done

python scripts/14_k_sensitivity.py \
  --output "${OUT_BASE}/figure2_k_sensitivity.csv" \
  --runs-dir "${OUT_BASE}" \
  --k-values "$(echo "${K_VALUES}" | tr ' ' ',')"

echo "K sensitivity closed-loop outputs written under ${OUT_BASE}"
